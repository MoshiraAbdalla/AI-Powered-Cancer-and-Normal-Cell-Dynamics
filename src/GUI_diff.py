...
def dea_from_tracks_paper_exact(linked_df, frame_col="frame",
    ...
    # resample tracks to integer frames
    series = []
    ...
    lo = q1 - 0.1 * span; hi = q99 + 0.1 * span
    ...
    if valid.sum() < 6:
        delta = np.nan; A = np.nan; start = 0
    else:
        idx = np.where(valid)[0]
        start = idx[len(idx) // 2]
        x = np.log(t_grid[start:]); y = S_vals[start:]
        delta, A = _theil_sen(x, y)
    ...
    if out_png is not None:
        ...
        # tmin, tmax = int(np.nanmin(t_grid)), int(np.nanmax(t_grid))
        # ticks = list(range(max(2,tmin), min(21,tmax)+1, 1)) \
        #       + list(range(max(22,tmin), min(51,tmax)+1, 2)) \
        #       + list(range(max(55,tmin), tmax+1, 5))
        # ticks = [t for t in ticks if tmin <= t <= tmax]
        # if ticks:
        #     ax.xaxis.set_major_locator(FixedLocator(ticks))
        #     ax.xaxis.set_major_formatter(FuncFormatter(lambda x, pos: f"{int(x)}"))
        #     ax.minorticks_off()

        # ttl = f"Diffusion Entropy Analysis (δ ≈ {delta:.3f})" if np.isfinite(delta) else "Diffusion Entropy Analysis"
        # ax.set_xlabel("Time window t"); ax.set_ylabel("S(t)"); ax.set_title(ttl)
...
def run_pipeline(video_path, results_root, log_fn=print, progress_fn=lambda *_: None, preview_fn=lambda *_: None):
    ...
    for pid, group in linked.groupby("particle"):
        ...
        speed = float(np.mean(np.sqrt(dx**2 + dy**2)))*fps  # px/frame (kept to match GUI)
    ...
    if len(feat_df) == 0:
        mean_speed = mean_disp = mean_angle = mean_P = 0.0
    else:
    ...
    # 6) Classification + DEA (with short-video tiling)
    ...
    votes["Normal" if mean_P > th_persist else "Cancer"] += 1; total_votes += 1

    # # DEA compute (tile short clips to extend range)
    MIN_FRAMES_FOR_DEA = 200
    # try:
    #     max_frame = int(linked["frame"].max()) if len(linked) else 0
    # except Exception:
    #     max_frame = 0
    # linked_for_dea = linked
    # if max_frame and max_frame < MIN_FRAMES_FOR_DEA:
    #     repeat_factor = int(np.ceil(MIN_FRAMES_FOR_DEA / max_frame))
    #     log_fn(f"[INFO] Repeating trajectories {repeat_factor}× for DEA.")
    #     tiles = []
    #     for r in range(repeat_factor):
    #         temp = linked.copy()
    #         temp["frame"] = temp["frame"] + r * (max_frame + 1)
    #         tiles.append(temp)
    #     linked_for_dea = pd.concat(tiles, ignore_index=True)
    linked_for_dea=linked
    dea_png = os.path.join(out_dir, "diffusion_entropy_dense.png")
    res = dea_from_tracks_paper_exact(linked_for_dea, out_png=dea_png)
    dea_delta = float('nan')
    if res is not None:
        _, _, dea_delta = res
        if np.isfinite(dea_delta):
            log_fn(f"[INFO] DEA slope δ = {dea_delta:.3f}")
            mu_S = 1.0 + 1.0 / dea_delta
            log_fn(f"[INFO] μ_S (from DEA) = {mu_S:.3f}")
            votes["Cancer" if dea_delta > th_dea else "Normal"] += 1
            total_votes += 1
            preview_fn({"type": "dea_plot", "path": dea_png})
        else:
            log_fn("[WARN] DEA vote skipped (no reliable asymptotic slope).")
    else:
        log_fn("[WARN] DEA skipped (insufficient tracks).")
    ...
    previews = {
        ...
        "mean_P": mean_P,
        "dea_png": dea_png,
        "dea_delta": float(dea_delta),
        # thresholds for GUI
        "th_speed": float(th_speed),
        "th_disp":  float(th_disp),
        "th_angle": float(th_angle),
        "th_persist": float(th_persist),
        "th_dea": float(th_dea),
    }
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.persist_var    = tk.StringVar(value="—")
        self.delta_var      = tk.StringVar(value="—")        # NEW: DEA δ
        self.thresh_var     = tk.StringVar(value="—")        # NEW: thresholds line

        self.img_pre = None; self.img_seg = None; self.img_track = None
        self.img_trajplot = None; self.img_dea = None
        self._live_running = False; self._live_thread = None
        self._live_writer = None; self._live_cap = None; self._live_path = None
        self.build_ui()
    ...
    def preview_cb(self, evt: dict):
        def _apply():
            ...
            if typ == "pre_frame":
                ...
                p = evt.get("path"); img = cv2.imread(p, cv2.IMREAD_COLOR)
                if img is not None:
                    self.img_dea = to_tk(img); self.lbl_dea.configure(image=self.img_dea, text=""); self.tabs.select(self.tab_dea)
            elif typ == "log":
    ...
    def build_ui(self):
        ...
        self.tab_dea = ttk.Frame(self.tabs, padding=pad)
        self.tabs.add(self.tab_dea, text="5) Diffusion Entropy")
        ttk.Label(self.tab_dea, text="Diffusion Entropy S(t) (All Cells)", style="Title.TLabel").pack(anchor="w", pady=(0,8))
        self.lbl_dea = ttk.Label(self.tab_dea, text="(Run to see diffusion entropy)", anchor="center")
        self.lbl_dea.pack(fill="both", expand=True, padx=24, pady=12)

        self.tab_cls = ttk.Frame(self.tabs, padding=pad); self.tabs.add(self.tab_cls, text="6) Classification")
        ...
        # ----- NEW: Decision Thresholds line (above metrics) -----
        ttk.Separator(self.tab_cls, orient="horizontal").pack(fill="x", pady=(10,10))
        # ttk.Label(self.tab_cls, text="Decision Thresholds", style="Title.TLabel").pack(anchor="w")
        # ttk.Label(self.tab_cls, textvariable=self.thresh_var, wraplength=1200).pack(anchor="w", pady=(0,10))

        # Metrics grid (now includes DEA δ)
        ...
        ttk.Label(stats, text="DEA slope δ:", style="StatLabel.TLabel").grid(row=5, column=0, sticky="w", padx=(0,12))  # NEW
        ttk.Label(stats, textvariable=self.delta_var, style="StatValue.TLabel").grid(row=5, column=1, sticky="w")       # NEW
    ...
    def run(self):
        try:
            ...
            for lbl, text in [(self.lbl_seg,"(Run to see segmentation overlay)"),
                              ...
                              (self.lbl_dea,"(Run to see diffusion entropy)")]:
            ...
            self.persist_var.set("—"); self.delta_var.set("—")
            self.thresh_var.set("—")
            ...
            # NEW: show DEA δ in metrics
            dea_delta = previews.get("dea_delta", float("nan"))
            self.delta_var.set(("n/a" if not np.isfinite(dea_delta) else f"{dea_delta:.3f}"))

            # NEW: thresholds line
            # th_speed   = previews.get("th_speed", 0.0)
            # th_disp    = previews.get("th_disp", 0.0)
            # th_angle   = previews.get("th_angle", 0.0)
            # th_persist = previews.get("th_persist", 0.5)
            # th_dea     = previews.get("th_dea", 0.5)
            # self.thresh_var.set(
            #     f"Speed > {th_speed:.2f} ⇒ Cancer | "
            #     f"Displacement > {th_disp:.2f} ⇒ Normal | "
            #     f"Angle > {th_angle:.2f}° ⇒ Cancer | "
            #     f"Persistence P > {th_persist:.2f} ⇒ Normal | "
            #     f"DEA δ > {th_dea:.2f} ⇒ Cancer"
            # )
