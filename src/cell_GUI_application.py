...
def build_segmentation_overlay(gray_path, mask_path):
    """Return a BGR image with a pink segmentation tint for debugging or export."""
...
def _particle_color(pid):
    """Deterministically generate a vibrant BGR color for a particle id."""
    digest = hashlib.sha1(str(pid).encode("utf-8", "ignore")).digest()
...
def build_tracking_overlay(base_frame_path, linked_df):
    """Return a BGR image with colored trajectories for debugging or export."""
    ...
    for pid, group in linked_df.groupby("particle"):
        ...
        pts = np.round(group[["x", "y"]].to_numpy()).astype(int)
        if len(pts) == 0:
            continue
...
def draw_tracks_overlay(base_frame_path, linked_df, size=(520, 380)):
    try:
        img = build_tracking_overlay(base_frame_path, linked_df)
    except Exception:
        return None
    if img is None:
        return None
